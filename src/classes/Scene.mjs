...
class Scene {
	constructor() {
		...
		this.shaderMaterial = new THREE.ShaderMaterial({
			...
			void main(){
					...
					// direction de la vue et la normal
					...
					// Shaping function
					inverseFresnelFactor = step(uFresnelWidth, fresnelFactor);

					vec3 color = mix(uBaseColor, uFresnelColor, fresnelFactor);

					gl_FragColor = vec4(color, fresnelFactor);
			}
			`,
			uniforms: {
	...
	tweaks = () => {
		...
		const widthInput = pane.addInput(this.params, 'fWidth', { min: 0, max: 1 });
		widthInput.on('change', (e) => {
			this.shaderMaterial.uniforms.uFresnelWidth.value = 1 - e.value;
		});
		const fresnelColorInput = pane.addInput(this.params, 'fColor');
		fresnelColorInput.on('change', (e) => {
